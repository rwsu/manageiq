module EmsRefresh
  module Parsers
    class OpenstackInfra < Infra
      ...
      def stacks
        @stacks ||= get_detailed_stacks
      end

      def get_detailed_stacks
        detailed_stacks = []
        # Seems like Heat doesn't suport detailed list, so we need to call get for each stack, which is suboptimal
        @orchestration_service.stacks.each {|stack| detailed_stacks << stack.details}
        detailed_stacks
      end
      ...
      def parse_host(host, indexed_servers, _indexed_hosts_ports, indexed_resources)
        uid = host.uuid
        hostname = identify_hostname(indexed_resources, host.instance_uuid)
        ...
        new_result = {
          ...
          :operating_system => {:name => hostname, :product_name => 'linux'},
          :vmm_vendor       => 'RedHat',
      ...
      def get_purpose(indexed_resources, instance_uuid)
        indexed_resources.try(:[], instance_uuid).try(:[], 'resource_name')
      end
      ...
      def identify_product(indexed_resources, instance_uuid)
        ...
        if purpose == 'NovaCompute'
          ...
          "rhel (No hypervisor, the purpose of the host is #{purpose})"
        end
      end

      def identify_hostname(indexed_resources, instance_uuid)
        purpose = get_purpose(indexed_resources, instance_uuid)
        return nil unless purpose

        "unknown hostname (#{purpose})"
      end
